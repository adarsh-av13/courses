...
for m in range(0, prime_list_length):
    for n in range(m+1, prime_list_length):
        x = prime_list[m] * prime_list[n]
        semi_prime_list.append(x)
#print(semi_prime_list)
semi_prime_length = len(semi_prime_list)


for i in range(0, semi_prime_length):
    for j in range(i+1, semi_prime_length):
        x = semi_prime_list[i] + semi_prime_list[j]
        my_list.append(x)
        if(x == num):
            flag = 1
